...
void lierda_App_task(void *param)
{
	...
	TimerControlFun(TIMEDREPORT,1,1000);//打开定时发送定时器
	for (;;)
	{
...
void lierda_ReadSensor(void)
{
	...
	lierda_HDC1000_UpdateInfo(&Lierda_SysValue.TemperSensor,&Lierda_SysValue.Humidity);//温湿度数据获取
}
...
static void NetworkConfigInit(void)
{
	...
	lierdaATDemoCall("AT+CPSMS=0","OK",3000,5);//关闭PSM模式
	lierdaATDemoCall("AT+CEDRXS=2,5,0101","OK",3000,5);//关闭EDRX
	osDelay(3000);
	lierdaATDemoCall("AT+CGATT=1","OK",2000,5);//打开自动联网
	do
	{
