...
#define	GPSDATAPACK_LEN	       17  //gps数据包长度
#define	TRACKER_LEN	       10  //定位器数据包前端长度
#define	TRACKERMODE_TRA    0X01 //定位器模式:定位
...
#define IMSI_LEN			15
#define ICCID_LEN			20
#define	SERVER_BUFF_MAXLEN	1024
...
struct Tracker_Pack
{
	uint8 Scenes;//场景
	uint8 mode;//模式
	uint8 InitInfo;//初始化信息
	uint8 AutoTrigger;//自动触发还是定时上报
	uint8 BatteryInfo;//电池电量
	uint8 AvailableTime;//可用时间
	uint8 DistanceSensor;//距离传感器标志位
	uint16 GpsDataLen;//GPS数据长度
	uint8 GpsDataNum;//GPS数据个数
}__attribute__((packed));


extern uint8 GpsDtaBuff[300];
