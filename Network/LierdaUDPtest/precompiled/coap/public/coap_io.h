...
typedef struct coap_endpoint_t {
  ...
  * this was added in a hurry, not sure it confirms to the overall model */
  ...
  int ifindex;
  int flags;
  struct ack_queue  *aq[ACK_QUEUE_LEN];
} coap_endpoint_t;

#define COAP_ENDPOINT_NOSEC 0x00
#define COAP_ENDPOINT_DTLS  0x01

coap_endpoint_t *coap_new_endpoint(coap_address_t *addr, int flags);
...
void coap_packet_get_memmapped(coap_packet_t *packet,
                               ...
                               size_t *length);
void coap_recv(void *arg, struct udp_pcb *upcb, struct pbuf *p, const ip_addr_t *addr, u16_t port);
