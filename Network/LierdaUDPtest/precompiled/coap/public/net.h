...
#include "pdu.h"
#include "prng.h"
#define RETRANSMIT_FILTER_TEST 0
#define COCOA 1
#define COAP_MESSAGE_CONSTANT_GAP 10000
struct coap_queue_t;
...
typedef struct coap_context_t {
  ...
  struct coap_resource_t *root_resource;
#ifndef WITHOUT_ASYNC
  /**
...
#ifdef COCOA   
   coap_tick_t E_strong;
   coap_tick_t E_weak;
   coap_tick_t rto;
   coap_tick_t SRTT;
   coap_tick_diff_t RTTVAR;
   unsigned char rto_state;
#endif
 ...
 */
coap_context_t *coap_new_context(coap_address_t *listen_addr);

/**
 * init the initial value of coap message id 
 * @param context The current coap_context_t object.
 */
void coap_message_id_init(coap_context_t *context);
 ...
 */
unsigned short coap_new_message_id(coap_context_t *context);
...
void coap_cancel_appdata_con_retrans_msg(coap_context_t *context);
void coap_cancel_upreg_con_retrans_msg(coap_context_t *context);
void coap_cancel_all_con_retrans_msg(coap_context_t *context);
/**
...
//coap_tid_t lierda_coap_send_impl(coap_context_t *context,
//	       const coap_endpoint_t *local_interface,
//	       const coap_address_t *dst,
//	       coap_pdu_t *pdu);

coap_tid_t
lierda_coap_send_confirmed(coap_context_t *context,
		    const coap_endpoint_t *local_interface,
		    const coap_address_t *dst,
		    coap_pdu_t *pdu);
