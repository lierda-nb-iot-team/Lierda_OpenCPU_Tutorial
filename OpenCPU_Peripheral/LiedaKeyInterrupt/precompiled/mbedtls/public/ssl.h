...
#define MBEDTLS_ERR_SSL_DISCARD_ERR_MSGTYPE               -0x8000  /**< 服务器连续发多条数据，网络异常导致前一数据丢失，接收到后一个数据包，程序不退出，由定时器超时重发处理. */
...
/* DTLS timer only for handshake */
#define MBEDTLS_SSL_DTLS_TIMEOUT 5
#define MBEDTLS_SSL_DTLS_RESEND_MAX_COUNT 3
...
#define MBEDTLS_SSL_MSG_UPDATE_RESUME_ID       24
#define MBEDTLS_SSL_MSG_UPDATE_RESUME_ID_RESPONSE     25
...
/*add for resume id 
 *increase DTLS Header lenth 
 *
 *
 */

#define MBEDTLS_HDR_CONTENTTYPE_LEN           (1)
#define MBEDTLS_HDR_VERSION_LEN               (2)
#define MBEDTLS_HDR_EPOCH_LEN                 (2)
#define MBEDTLS_HDR_SEQUENCE_LEN              (6)
#define MBEDTLS_HDR_RESUMEID_LEN              (8)
#define MBEDTLS_HDR_LENGTH_LEN                (2)

//8
#define MBEDTLS_HDR_CTR_LEN                   (MBEDTLS_HDR_EPOCH_LEN + MBEDTLS_HDR_SEQUENCE_LEN)
 
#define MBEDTLS_HDR_STANDARD_LEN              (13)
...
/*
 * SSL state machine for handshake
 */
typedef enum
{
...
typedef struct dtls_pcb dtls_pcb;
...
typedef void (*dtls_recv_fn)(void *arg, struct udp_pcb *pcb, struct pbuf *p,
...
struct mbedtls_ssl_context
{
    ...
    // entrance pointer that resumeid position 
    unsigned char *in_resumeid; /*!< 8-bytes resume id length field   */     
    ...
    /*!< 3-10  DTLS: epoch + record sequence number*/
    /*!< 3-4   DTLS: epoch */
    /*!< 5-10  DTLS: record sequence number*/

    unsigned char *out_ctr;     /*!< 64-bit outgoing message counter  */
    
    /*!< 0-0   message type, = out_buf*/
    /*!< 1-2   protocol version*/
    unsigned char *out_hdr;     /*!< start of record header           */

    // exit pointer that resumeid position 
    unsigned char *out_resumeid; /*!< 8-bytes resume id length field   */     
...
/* the dtls handshake timer */
extern int dtls_handshake_resend_cnt;
extern timer_handle dtls_handshake_timer;
extern mbedtls_ssl_context *dtls_timer_ssl;

typedef struct dtls_address_t {
  uint16_t port;
  ip_addr_t addr;
} dtls_address_t;

/* Payload Confidentiality Block*/
typedef struct dtls_pcb {
  struct udp_pcb *upcb;
  mbedtls_ssl_context *dtls_ssl;
  uint8 handshake_fail_count;
  dtls_recv_fn recv;
  void *recv_arg;
  dtls_address_t dtls_conn_addr;
} dtls_pcb;
